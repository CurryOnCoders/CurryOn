...
module OperationBuilder =
    ...
    type Task<'a> with
        static member FromException(ex: exn) =
            ...
            tcs.SetException(ex)
            tcs.Task
        static member FromFailureEvents<'a, 'e>(events: 'e list) =
            Task.FromException<'a * 'e list>(events |> failEx)   
        static member FromFailure<'a, 'e>(events: 'e list) =
    ...
    let rec mergeEvents<'result, 'event> (step: OperationStep<'result,'event>) (events: 'event list) =
        match step with
        | CompletedStep result -> 
            CompletedStep <| Result.mergeEvents result events
        | InProcessStep inProcess -> 
            let inProcessTask =
                inProcess.ContinueWith(fun (task: Task<'result*'event list>) -> 
                    if task.IsFaulted then 
                        let taskEvents =
                            match Result.ofException task.Exception with
                            | Failure es -> es
                            | _ -> []
                        Task.FromFailureEvents<'result,'event> (events @ taskEvents)
                    elif task.IsCanceled then 
                        Task.FromFailureEvents<'result,'event> events
                    else 
                        let result,taskEvents = task.Result
                        Task.FromResult<'result*'event list>(result, (events @ taskEvents))).Unwrap()
            inProcessTask |> InProcessStep
        | AsyncStep async -> 
            AsyncStep {async with Continuation = (fun () -> async.Continuation() |> mergeEvents<'result, 'event> <| events)}
        | LazyStep deferred -> 
            lazy(mergeEvents<'result, 'event> deferred.Value events) |> EventingLazy |> LazyStep
    ...
    let bindAsyncResult (asyncVal: Async<OperationResult<'a,'event>>) (continuation: 'a -> OperationStep<'b, 'event>) =
        let task = 
            async {
                ...
                | Failure errors -> return! errors |> Task.FromFailure<'a, 'event> |> Async.AwaitTask
            } |> Async.StartAsTask
    ...
    let bindParallel (asyncVal: Async<OperationResult<'a,'event> []>) (continuation: 'a list -> OperationStep<'b, 'event>) =
        let task = 
            async {
                ...
                | Success successfulResult -> return successfulResult.Result
                | Failure errors -> return! errors |> Task.FromFailure<'a list, 'event> |> Async.AwaitTask
            } |> Async.StartAsTask
        bindTaskNoContext task continuation
    ...
    let runAsTask (firstStep : unit -> OperationStep<'result,'event>) =
        InProcess <| Task.Run(fun () -> run firstStep).ContinueWith(fun (task: Task<Operation<'result,'event>>) ->
            if task.IsFaulted then 
                Task.FromException<'result*'event list>(task.Exception)
            elif task.IsCanceled then 
                Task.FromException<'result*'event list> <| OperationCanceledException()
            else 
                let rec toTask operation =            
                    match operation with
                    | Completed result -> 
                        match result with
                        | Success successfulResult -> Task.FromResult(successfulResult.Result,successfulResult.Events)
                        | Failure errors -> Task.FromFailureEvents<'result, 'event> errors
                    | InProcess t -> t
                    | Cancelled events-> Task.FromFailureEvents<'result, 'event> events
    ...
    let returnAsync (asyncVal: Async<OperationResult<'result,'event>>) =
        let rawAsync = 
            async {
                let! result = asyncVal
                match result with
                | Success successfulResult -> 
                    return successfulResult.Result,successfulResult.Events
                | Failure errors -> 
                    return! errors |> Task.FromFailureEvents<'result, 'event> |> Async.AwaitTask
            }
        rawAsync |> Async.StartAsTask |> InProcessStep
    ...
    let returnParallel (asyncVal: Async<OperationResult<'result,'event> []>) =
        let rawAsync = 
            async {
                let! results = asyncVal
                match results |> Result.collect with
                | Success successfulResult -> return successfulResult.Result,successfulResult.Events
                | Failure errors -> return! errors |> Task.FromFailure<'result, 'event> |> Async.AwaitTask
            }
...
module OperationBindings =
    ...
    let railway = OperationBuilder.OperationBuilder()
